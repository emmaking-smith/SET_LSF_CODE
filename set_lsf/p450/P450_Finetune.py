...
import P450_df_setup as ds
import P450_Setup as ms
import matplotlib.pyplot as plt

from LSF_Finetune_Net import LSF_MPNN

def init_args():
    '''
    Setting up the input arguments.
    Takes the path to the npz data and the save path for the
...
def main():
    ...
    # Set up logger.
    log_path = os.path.join(save_path, 'p450_model_log.log')
    ...
    # Reading in the standardized csv file.
    ...
    # atom_list is now ordered from most common atom to least common atom.
    atom_list = ds.total_atom_types(df)
    ...
    # Finding the complete list of unique P450s in the dataframe
    unique_p450s = df['P450'].unique().tolist()

    # Setting up the data.
    # test_data, train_data = ds.pfizer_test_mol(df, test_index)
    # test_data, train_data = ds.test_molecules(df, 1) # Only using most common molecule for testing.
    ...
    train_data = ms.Pfizer_Dataset(train_data, longest_molecule, atom_list, unique_reagents, unique_oxidants,
                 unique_solvents, unique_acids, unique_additives, unique_p450s)
    ...
    atom_num_list = ds.total_atom_numbers(atom_list)
    ...
    preds_save_path = os.path.join(save_path, 'p450_pred.pickle')
