...
def rxn_encoding(df_row, unique_reagents, unique_oxidants, unique_solvents, unique_acids, unique_additives, unique_p450s):
    '''
    ...
    we are one-hot encoding is: solvent, additive, oxidant, acid, radical mechanism.
        Args:
            ...
            unique_p450s (list of strings):
                A list which contains all the unique P450s possible in the dataframe.
...
class Pfizer_Dataset(Dataset):
    ...
    def __init__(self, df, longest_molecule, atom_list, unique_reagents, unique_oxidants,
        ...
        self.unique_p450s = unique_p450s
    ...
    def __getitem__(self, index):
        ...
        rxn_vector = rxn_encoding(self.df.iloc[index], self.unique_reagents, self.unique_oxidants, self.unique_solvents,
                                  self.unique_acids, self.unique_additives, self.unique_p450s)
...
def collate_y(batch):
    ...
    for i in range(1, batch_size):
        ...
        # Concatonating all the unpad_rxn_vectors's together.
    ...
    # Reshaping the Y's to be a long 1D tensor.
    Y = Y.view([-1])
