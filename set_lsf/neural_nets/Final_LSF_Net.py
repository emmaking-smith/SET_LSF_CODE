'''
Finetuning on the top 4 atom GRU 4 linear layer network.
'''
...
def gen_states(model_state_dict):
    ...
    for k in states.keys():
        if 'selectivity' in k:
            name = k.split('selectivity.')[1]
            new_state_dict[name] = states[k]
    return new_state_dict

class LSF_MPNN(nn.Module):
   
    def __init__(self, message_size, message_passes, all_unique_atoms, rxn_features_length, model_path):
